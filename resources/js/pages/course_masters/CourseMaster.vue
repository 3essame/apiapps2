	...
	<v-card>
		...
		<v-card-text>
			...
			<v-list subheader>
				...
				<template v-for="(department,index) in master.departments">

					<v-list-item :key="master.id">
						...
						<v-chip class="mx-2" small label @click.stop="editCourseMaster(department.id)">تعديل</v-chip>
						<v-chip color="error" small label @click.stop="deleteCourseMaster(department.id)">حذف</v-chip>
				...
				<v-subheader>الدورات</v-subheader>

				<template v-for="(course,index) in master.courses">
					...
					<v-list-item :key="master.id">
						...
						<v-chip class="mx-2" small label @click.stop="editCourseMaster(course.id)">تعديل</v-chip>
						<v-chip color="error" small label @click.stop="deleteCourseMaster(course.id)">حذف</v-chip>
...
<script>
	...
	export default {
		...
		methods: {
			fechCourseMaster() {
				axios.get('course-master/' + this.$route.params.id).then(res => this.master = res.data)
			},
